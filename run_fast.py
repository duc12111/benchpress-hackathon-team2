...
# from utilities import run_test_cases, score
# import prompting
# import re
...
# def generate_code_solutions(problem: dict, num_samples: int, temperature: float, start_time: float, time_limit: float) -> List[str]:
#     """
...
#                 print("Time limit approaching during code generation, returning best code solutions found so far.")
#                 break
...
# def select_best_code_full(code_solutions: List[str], test_cases: List[Dict[str, Any]], problem: dict, start_time: float, time_limit: float) -> str:
#     """
...
#         elapsed_time = time.time() - start_time
#         if elapsed_time >= time_limit - 10:  # Stop if less than 10 seconds remain
#             print("Time limit approaching during code evaluation, selecting best code so far.")
#             break
...
#         return code_solutions[0] if code_solutions else ""  # Fallback
...
#     code_solutions = generate_code_solutions(problem, num_code_samples, temperature, start_time, time_limit)
...
#     best_code = select_best_code_full(code_solutions, test_cases, problem, start_time, time_limit)

#     return best_code

# def main():
#     parser = argparse.ArgumentParser(description='CODET Benchmark Script')
#     parser.add_argument('--num_code_samples', type=int, default=100, help='Number of code samples to generate')
...
#             args.temperature,
#             time_limit=300.0  # 5 minutes per problem
#         ),
#         client=client,
...
def select_best_code_full(code_solutions: List[str], test_cases: List[Dict[str, Any]], problem: dict, start_time: float, time_limit: float) -> str:
    """
    Select the best code solution by evaluating all code solutions on all test cases.
    If a code passes all test cases, it is immediately returned to improve speed.
...
def main():
    parser = argparse.ArgumentParser(description='CODET Benchmark Script')
    parser.add_argument('--num_code_samples', type=int, default=50, help='Number of code samples to generate')
    parser.add_argument('--temperature', type=float, default=0.8, help='Sampling temperature for diversity')
    parser.add_argument('--length', type=int, default=1, help='Number of problems to evaluate')
    args = parser.parse_args()
